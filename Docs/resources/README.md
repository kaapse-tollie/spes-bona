...
- Maintained evidence tables are now append-only during the state-audit loop: lifecycle columns preserve superseded and rejected references instead of deleting them.
...
- The same sheet also shows the current per-state audit progress block for the stateful review loop.
...
2. run exactly one state audit pass:
...
If the package needs a plain rebuild without advancing the tracker:
...
The stateful loop keeps the maintained evidence rows append-only and advances one SB state per `state-pass`. `build` regenerates the workbook and CSV outputs without advancing the tracker. `sync-live` still exists for manual use, but `state-pass` already performs the build/test/sync/test sequence for the selected state.
