...
| `OB1-WAR-08` | Back down once as initiator and once as target before war in Natal and Martinus routes. Force a scripted play-creation failure while an unrelated/old play exists. | The non-backing side's demand enters the same resolver immediately; no state waits for `on_war_end`; no goal/backer is added to the unrelated play; every launch lease and pending bit clears. | Engine pending |
| `OB1-WAR-09` | Repeat capitulation, negotiated peace, and white peace across Natal crisis, British–Zulu annexation/handoff, Klip River, Martinus, Kimberley, Xhosa/frontier wars, Swazi/Zulu, Great Trek, BST, NRP boundary, and Bechuanaland. | Every dispatch requires its play type and authored route marker. Timer callbacks are nonterminal, final predicates own the result, and each route resolves idempotently. | Engine pending |
