...
class NatalZululandStateSplitTests(unittest.TestCase):
    ...
    def test_fresh_1836_ownership_population_and_building_history(self):
        ...
        ngi_buildings = scoped_block_from_source(natal_buildings, "region_state:NGI")
        cap_buildings = scoped_block_from_source(natal_buildings, "region_state:CAP")
        self.assertIn("region_state:NGI", natal_buildings)
        self.assertIn("sb_purge_starting_buildings_effect = yes", ngi_buildings)
        self.assertNotIn("create_building", ngi_buildings)
        self.assertNotIn("building_livestock_ranch", ngi_buildings)
        self.assertIn("region_state:CAP", natal_buildings)
        self.assertNotIn("region_state:ZUL", natal_buildings)
        self.assertIn('building = "building_port"', cap_buildings)
        self.assertIn('country = "c:CAP"', cap_buildings)
