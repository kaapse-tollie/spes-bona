...
def release_archive(version="1.65.0", extra=None):
    output = io.BytesIO()
...
def fake_opener(version="1.65.0", archive=None, digest=None):
...
class CmfSynchronizationTests(unittest.TestCase):
    def test_latest_release_is_installed_even_across_minor_lines(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            opener, calls = fake_opener("1.65.0")
            status, version = sync_cmf.synchronize(target, opener=opener)

            self.assertEqual(("updated", "1.65.0"), (status, version))
    ...
    def test_current_release_queries_github_without_redownloading(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            (target / ".metadata/metadata.json").write_text(
                json.dumps({"id": sync_cmf.CMF_ID, "version": "1.65.0"})
            )
            opener, calls = fake_opener("1.65.0")

            self.assertEqual(("current", "1.65.0"), sync_cmf.synchronize(target, opener=opener))
    ...
    def test_digest_failure_preserves_existing_install(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            opener, _calls = fake_opener("1.65.0", digest="0" * 64)
