...
class OverrideInventoryTests(unittest.TestCase):
    ...
    def test_release_metadata_and_dependency_are_locked(self):
        ...
        metadata["relationships"][0]["version"] = "1.61.0"
        ...
        self.assertTrue(any("version 1.61.*" in error for error in errors))
    ...
    def test_cmf_1_61_api_surface_is_locked(self):
        ...
        (cmf / ".metadata/metadata.json").write_text(json.dumps({"version": "1.61.0"}))
        ...
        self.assertTrue(any("CMF 1.61.0 API com_container" in error for error in errors))
